...
.PHONY: install run test migrate docker-build docker-run db-start lint migrate-server run-server vagrant-build vagrant-migrate-server vagrant-run vagrant-db-start
